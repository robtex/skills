# Windsurf rules snippet

Paste the block below into a `.windsurfrules` file at the root of your project. Windsurf's Cascade agent will load it for every AI interaction in that workspace.

## Why

Windsurf's Cascade agent will confidently invent DNS records, IP reputations, AS numbers, and Bitcoin data. This rule forces it to call the free Robtex MCP server (44 tools, no key, no signup) instead of hallucinating network and blockchain data.

## Install

1. Create `.windsurfrules` in your project root (or append to an existing one).
