...
Paste the block below into a `.clinerules` file at the root of your project. Cline will load it as system instructions for every task in that workspace.

## Why

Cline will make up DNS records, IP reputations, AS numbers, and Bitcoin balances when given networking or OSINT tasks. This rule forces it to call the free Robtex MCP server (44 tools, no key, no signup) instead of hallucinating.

## Install

1. Create `.clinerules` in your project root (or append to an existing one).
